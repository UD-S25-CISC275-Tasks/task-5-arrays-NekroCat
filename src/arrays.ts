...
export function bookEndList(numbers: number[]): number[] {
    ...
    if (numbers.length === 0) {
        ...
        let numbers3: number[] = [...numbers2, numbers[0]];
        let numbers4: number[] = [...numbers3, numbers[0]];
        return numbers4;
    } else {
        let numbers3: number[] = [...numbers2, numbers[0]];
        let numbers4: number[] = [...numbers3, numbers[numbers.length - 1]];
        return numbers4;
    }
}
...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map((amount) => {
        let cleanedAmount: string = amount;

        if (amount.startsWith("$")) {
            cleanedAmount = amount.slice(1);
        }

        let parsed: number = parseInt(cleanedAmount);
...
export function makeMath(addends: number[]): string {
    ...
    let numbersString: string = addends.join("+");
